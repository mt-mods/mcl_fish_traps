...
local trap = {
	...
	on_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		core.log("action", player:get_player_name() .. " moves stuff in fishing trap at " .. core.pos_to_string(pos))
	end,
	on_metadata_inventory_put = function(pos, listname, index, stack, player)
		core.log("action", player:get_player_name() .. " moves stuff to fishing trap at " .. core.pos_to_string(pos))
	end,
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		core.log("action", player:get_player_name() .. " takes stuff from fishing trap at " .. core.pos_to_string(pos))
	end,
...
-- Nodes Registration
core.register_node("mcl_fish_traps:fishing_trap", trap)
core.register_node("mcl_fish_traps:fishing_trap_water", trap_w)

-- Register Fish Trap Crafting Recipe
local easy_recipe = core.settings:get("mcl_fish_traps_easy_recipe") or false
local trap_recipe
if easy_recipe then
	trap_recipe = {
	...
	local panes = (game.id == "mineclone2" or game.id == "VoxeLibre") and "xpanes:bar_flat" or "mcl_panes:bar_flat"
	trap_recipe = {
...
local wet_time = core.settings:get("mcl_fish_traps_wet_time") or 20
core.register_abm({
	...
	action = function(pos, value)
		for _, v in pairs(adjacents) do
			...
			if core.get_item_group(n, "water") > 0 then
				core.swap_node(pos, { name = "mcl_fish_traps:fishing_trap_water" })
				return
			end
		end
	end,
...
if game.id == "mineclone2" or game.id == "VoxeLibre" then
	loot_table = {
		...
		treasure = {
			...
			{
				itemstring = "mcl_fishing:fishing_rod",
				wear_min = 49144,
				wear_max = 65535,
				func = function(stack, pr)
					mcl_enchanting.enchant_randomly(stack, 30, true, false, false, pr)
...
local drop_full = core.settings:get("mcl_fish_traps_drop_when_full") or false
local trap_wait = core.settings:get("mcl_fish_traps_wait") or 30
local run_chance = core.settings:get("mcl_fish_traps_chance") or 2
	...
	action = function(pos, value)
		for i, v in pairs(adjacents) do
			...
			if core.get_item_group(n, "water") > 0 then
				break
			elseif i > 3 then
				core.swap_node(pos, { name = "mcl_fish_traps:fishing_trap" })
				return
			end
		end
		local meta = core.get_meta(pos)
